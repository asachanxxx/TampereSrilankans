...
export async function GET(request: NextRequest) {
  try {
    ...
    if (ticketNumber) {
      ...
      // Only return registration PII (WhatsApp, spouse, children) to authenticated staff
      const currentUser = await getCurrentUser();
      const isStaff = currentUser && ['admin', 'moderator', 'organizer'].includes(currentUser.role);
